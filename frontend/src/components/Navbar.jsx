            ...
            {isAuthenticated && wallet && (
              <>
                ...
                {riskLevel && !dashboardData?.trust_score && (
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${riskLevel === 'low' ? 'risk-low' : riskLevel === 'medium' ? 'risk-medium' : 'risk-high'
                    }`}>
                    {riskLevel.toUpperCase()}
                  </span>
                )}
                <div className="flex items-center gap-2 bg-white/[0.03] border border-white/10 rounded-lg px-3 py-1.5">
                  <div className={`w-2 h-2 rounded-full ${riskColor[riskLevel] || 'bg-emerald-500'} pulse-dot`} />
